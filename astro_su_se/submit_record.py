...
class Record:
	...
	def get_oid(self, metadata=None):
		...
		if self.oid:
			...
			try:
				...
				raise ValueError('Cannot parse keyword "%s" into an oid: %s' % (DATE_KEYWORD, error)) from error
			except KeyError as error:
				raise ValueError('Keyword "%s" missing in FITS header, cannot generate default oid' % DATE_KEYWORD) from error
	...
	def get_metadata(self):
		...
		metadata = {'fits_header': fits_header.tostring().strip()}
...
if __name__ == '__main__':
	...
	parser.add_argument(
		'--file-path', metavar='FILE PATH', default=argparse.SUPPRESS, help='The relative path of the file'
	)
	parser.add_argument(
		'--thumbnail-url', metavar='THUMBNAIL URL', default=argparse.SUPPRESS, help='The URL of the thumbnail'
	)
	offline_parser = parser.add_mutually_exclusive_group(required=False)
	offline_parser.add_argument(
		'--offline', dest='offline', default=argparse.SUPPRESS, action='store_true', help='Set the record as offline'
	)
	offline_parser.add_argument(
		'--online',
		dest='offline',
	...
	parser.add_argument(
		'--username', '-u', default=argparse.SUPPRESS, help='The username (email) of the user owning the data'
	)
	...
	try:
		...
		logging.critical('Could not submit data: %s', error)
		if getattr(error, 'content', False):
