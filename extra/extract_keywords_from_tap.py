...
if __name__ == '__main__':
	...
	if args.max_distinct_values > 1:
		for keyword_info in keyword_infos:
			keyword_info['distinct_values'] = keyword_inspector.get_distinct_values(
				keyword_info['verbose_name'], keyword_info['sample_values']
			)

	try:
		with open(args.output, 'tw', encoding='UTF-8') as output_file:
			json.dump([], output_file, ensure_ascii=False, indent='\t')
