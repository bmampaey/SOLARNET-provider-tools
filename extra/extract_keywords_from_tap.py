...
class KeywordInspector:
	...
	def __init__(self, url, exclude_columns = []):
		...
		table_name = self.resolve_ambiguity(
			'Select the table to inspect',
			[table.name for table in service.tables]
		)
		self.table = service.tables[table_name]
		...
		for column in self.table.columns:
			...
			if column.name in self.exclude_columns:
				...
				keyword_infos.append({
					...
					'type': self.get_keyword_type(column),
	...
	def get_keyword_type(self, column):
		...
		if keyword_type is None:
			keyword_type = self.resolve_ambiguity(
				'Column "{column.name}" datatype "{column.datatype.content}" is not supported, select appropriate SVO keyword type'.format(column=column),
				['text', 'boolean', 'integer', 'real', 'time (ISO 8601)']
			)
		
		if keyword_type != 'time (ISO 8601)' and 'time' in column.ucd:
			keyword_type = self.resolve_ambiguity(
				'Column "{column.name}" has ucd "{column.ucd}", select appropriate SVO keyword type'.format(column=column),
				['time (ISO 8601)', keyword_type]
			)
		
		if 'x' in column.datatype.arraysize or (column.datatype.arraysize != '1' and keyword_type not in ['text', 'time (ISO 8601)']):
			keyword_type = self.resolve_ambiguity(
				'Column "{column.name}" datatype "{column.datatype.content}" with arraysize {column.datatype.arraysize} is not supported, select appropriate SVO keyword type'.format(column=column),
				['text', 'boolean', 'integer', 'real', 'time (ISO 8601)']
			)
...
if __name__ == "__main__":
	...
	# Get the arguments
	parser = argparse.ArgumentParser(description = 'Query a TAP service to extract the keywords definitions for the SOLARNET Virtual Observatory.')
	...
	parser.add_argument('--exclude', '-E', metavar = 'COLUMN', default = [], action = 'append', help = 'Columns to exclude, can be specified multiple times')
