...
class MetadataRecord:
	'''Class to extract the metadata from a FITS file and generate a dict of info for updating the SVO'''
	...
	def get_metadata(self):
		'''Return a dict of info for creating a new metadata record'''
		
		# Create the metadata dict with the required keywords and use the metadata
		# keyword definitions to extract the value for the rest of the fields
		metadata = {field: self.get_field_value(field) for field in ('oid', 'fits_header', 'date_beg', 'date_end', 'wavemin', 'wavemax')}
		
		for name in self.keywords:
			try:
				metadata[name] = self.get_field_value(name)
			except ValueError as why:
				logging.warning('Could not extract value for field %s: %s', name, why)
			else:
				logging.debug('Field %s has value "%s"', name, metadata[name])
...
class DataLocationRecord:
	'''Class to generate a dict of info for updating the SVO'''
	...
	def get_file_size(self):
		...
		if self.fits_file:
			...
			raise ValueError('Either file_size or fits_file must be set')
	...
	def get_file_path(self):
		...
		if self.file_path:
			...
			file_path = self.fits_file
			if self.BASE_FILE_DIRECTORY:
				file_path = os.path.realpath(file_path)[len(self.BASE_FILE_DIRECTORY):]
		else:
			raise ValueError('Either file_path or fits_file must be set')
	...
	def get_data_location(self):
		'''Return the dict of info for creating a new data location record'''
