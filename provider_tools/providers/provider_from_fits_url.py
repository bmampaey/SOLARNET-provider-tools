...
class ProviderFromFitsUrl(Provider):
	...
	Attributes:
		HEADER_SIZE (int): Size of the FITS header to read.
		HEADER_OFFSET (int): Byte offset from which to read the FITS header.
		ZIPPED (bool): Whether the FITS file is compressed.
		WEBSERVER_AUTH (tuple|None): Authentication credentials for the web server.
		METADATA_CLASS (MetadataFromFitsHeader): A subclass of Metadata used to build the metadata resource.
		DATA_LOCATION_CLASS (DataLocationFromUrl): A subclass of DataLocation used to build the data_location resource.
	"""
