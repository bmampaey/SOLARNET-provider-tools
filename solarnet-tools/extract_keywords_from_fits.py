...
class KeywordInspector:
		...
		for fits_file in self.fits_files:
			if fits_file in self.processed_fits_files:
				...
				try:
					...
					self.log.error('Could not open file %s: %s . Skipping!', fits_file, why)
	...
	def inspect_header(self, header):
		...
		for card in header.cards:
			try:
				...
				self.log.error('Could not parse card %s: %s . Skipping!', card, why)
				continue
	...
	def resolve_ambiguity(self, keyword, subject, values):
		...
		options = sorted(((sum(counter.values()), option, counter.most_common(3)) for option, counter in values.items()), reverse=True)
	...
	def save_backup(self):
		...
		if self.backup_file_path:
			...
			try:
				...
				self.log.error('Could not open backup file %s for writing: %s', self.backup_file_path, why)
				raise
			except Exception as why:
				self.log.error('Could not save state to backup file: %s', why)
				raise
	...
	def restore_backup(self):
		...
		if self.backup_file_path and os.path.isfile(self.backup_file_path):
			try:
				...
				self.log.error('Could not open backup file %s for reading: %s', self.backup_file_path, why)
				raise
			except Exception as why:
				self.log.error('Could not restore state from backup file: %s', why)
				raise

if __name__ == "__main__":

	# Get the arguments
	parser = argparse.ArgumentParser(description = 'Inspect the header of one or more FITS files and extract the keywords definitions for the SOLARNET Virtual Observatory. If possible try to use files spread over the entire dataset, instead of consecutive files, this will allow to detect changes over time in the header of the files.')
	parser.add_argument('fits_files', metavar = 'FITSFILE', nargs = '+', help = 'Path or URL to a FITS file to inspect')
	parser.add_argument('--hdu', default = '0', help = 'The index or name of the HDU to inspect (if not specified the first HDU will be inspected)')
	...
	try:
		...
		logging.critical('Fatal error: %s' % why)
		raise
	...
	try:
		...
		logging.critical('Could not open file %s for writing: %s' % (args.output, why))
		raise
	except Exception as why:
		logging.critical('Fatal error: %s' % why)
		raise
