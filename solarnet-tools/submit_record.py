...
# Set with proper relative base directory of data (can be left empty)
...
# If data should be offline by default
DEFAULT_OFFLINE = False

# URL of the RESTful API of the SVO
# Don't change this
API_URL = 'http://solarnet.oma.be/service/api/v1'


class TastypieApiKeyAuth(AuthBase):
	'''Sets the appropriate authentication headers for the RESTful API'''
...
class Record:
	def __init__(self, dataset, fits_file, file_url = None, file_path = None, thumbnail_url = None, oid = None, offline = DEFAULT_OFFLINE, username = DEFAULT_USERNAME, api_key = DEFAULT_API_KEY, **kwargs):
		...
		self.api = API(API_URL, auth = TastypieApiKeyAuth(username, api_key))
...
if __name__ == "__main__":
	...
	# Get the arguments
	parser = argparse.ArgumentParser(description='Submit meta-data from a FITS file to the SVO')
	parser.add_argument('--debug', '-d', action='store_true', help='Set the logging level to debug')
	parser.add_argument('dataset', help='The dataset ID in the SVO')
